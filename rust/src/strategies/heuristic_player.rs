...
pub struct HeuristicPlayer<'a> {
  heuristic: &'a dyn Fn(&Board) -> i32,
}
impl<'a> HeuristicPlayer<'a> {
  pub fn new (heuristic: &'a dyn Fn(&Board) -> i32) -> HeuristicPlayer {
    HeuristicPlayer {
      heuristic
    }
  }
  pub fn boxed_new (heuristic: &'a dyn Fn(&Board) -> i32) -> Box<HeuristicPlayer> {
    Box::new(Self::new(heuristic))
  }

  fn search_action (&self, depth:i32, board:&Board) -> ActionEval {
    if board.is_ended() {
      return ActionEval { value:board.ended_value(board.now_player), action:None };
    }
    if depth == 0 {
